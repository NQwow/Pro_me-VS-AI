...
def generate_ending(player, history):
    """结局生成：先用本地规则生成详细草稿，再让 AI 润色（若可用）"""
    # 1. 生成本地详细草稿（保证内容丰富、逻辑正确，并融入历史事件）
...
def _generate_detailed_ending_by_rules(player, history):
    """基于规则生成详细的结局草稿（约200-300字），融入历史关键事件"""
    ...
    # 提取历史关键事件（最多3个）
    key_events = _extract_key_events(history)
...
def _extract_key_events(history):
    """从历史记录中提取关键事件（最多3条）"""
    if not history:
        return []
    events = []
    for record in history[-5:]:  # 只看最后5回合
        # 提取属性变化较大的部分
...
def _get_default_ending(player):
    """原有的详细本地结局（作为最终 fallback，但通常不会被调用）"""
    w, s, h, l = player.wealth, player.skill, player.health, player.luck
...
晚景：{details[0]} {details[1]} {details[2]} {details[3]}

{player.name} 常对后辈说：“在这个 Python 能力贬值、AI 泛滥的世界，能保持初心已是万幸。”
最终，{player.name} 在{'睡梦中安详离世' if h > 50 else '病榻上留下遗憾' if h < 30 else '家人的陪伴下告别'}，享年{'八十有' + str(50 + (w//10)) if w>50 else '六十'}。他的故事，成为了那个特殊时代的一个注脚。"""
...
def _call_zhipu_api(player, round_num, memory, last_choice_result):
    ...
    print(" 智谱 AI 正在思考剧情", end="", flush=True)
    for _ in range(3):
        time.sleep(0.4)
